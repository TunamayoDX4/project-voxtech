...
const face_indices = array<array<u32, 4>, 6>(
  array<u32, 4>(4u, 0u, 3u, 7u), // West(-X face)
  array<u32, 4>(1u, 5u, 6u, 2u), // East(+X face)
  array<u32, 4>(0u, 1u, 2u, 3u), // South(-Y face)
  array<u32, 4>(5u, 4u, 7u, 6u), // North(+Y face)
  array<u32, 4>(4u, 5u, 1u, 0u), // Bottom(-Z face)
...
fn task_main(
  ...
  return vec3<u32>(64u, 64u, 6u); // 4プリミティブワークグループ
}
...
fn mesh_main(
  ...
  for (var i: u32 = 0u; i < 4u; i = i + 1u) {
    let index = face_indices[glob_id.z / 4u % 6u];
    let vert = 
      vec4<f32>(positions[index[i]] + vec4<u32>(
        loc_idx % 4u * position_offset.x,
        loc_idx / 4u % 4u * position_offset.y,
        loc_idx / 16u * position_offset.z,
        0u
      ) + vec4<u32>(
        glob_id.x / 4u % 4u,
        glob_id.x / 16u % 4u,
        ...
        task_payload.cell_position[glob_id.y / 4u % 64u], 0u
      ));
